...
const NetworkSecurity: React.FC = () => {
  ...
  return (
    <div className="panel-bg cyber-border h-48 flex flex-col">
      <div className="p-2 border-b border-cyan-500/20 px-3 flex justify-between items-center">
        <h2 className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Global Node Network</h2>
        <span className="text-[9px] font-mono text-cyan-500">REALTIME_MAP</span>
      </div>
      <div className="flex-1 p-2 relative bg-obsidian">
        <svg className="w-full h-full opacity-70" viewBox="0 0 200 100">
          ...
          {/* Animated pings */}
          {activeNodes.includes('node-eu') && <circle cx="45" cy="35" r="8" fill="none" stroke="#ef4444" strokeWidth="0.5" className="animate-ping"></circle>}
          {activeNodes.includes('node-us') && <circle cx="155" cy="45" r="8" fill="none" stroke="#00f2fe" strokeWidth="0.5" className="animate-ping"></circle>}
          {activeNodes.includes('node-asia') && <circle cx="100" cy="65" r="8" fill="none" stroke="#00f2fe" strokeWidth="0.5" className="animate-ping"></circle>}
        </svg>
        <div className="absolute bottom-2 left-2 flex flex-col gap-1">
           <div className="flex items-center gap-1">
              <div className="w-1 h-1 rounded-full bg-cyan-500"></div>
              <span className="text-[7px] font-mono text-slate-500 uppercase">Traffic_Pings</span>
           </div>
           <div className="flex items-center gap-1">
              <div className="w-1 h-1 rounded-full bg-red-500"></div>
              <span className="text-[7px] font-mono text-slate-500 uppercase">Threat_Activity</span>
           </div>
        </div>
      </div>
    </div>
